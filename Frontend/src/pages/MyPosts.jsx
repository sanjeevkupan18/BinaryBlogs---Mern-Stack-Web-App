...
const MyPosts = ({ onBack, onCreate, onSelectPost }) => {
  ...
  useEffect(() => {
    const loadPosts = async () => {
      try {
        const data = await apiRequest("/api/posts/me");
        setPosts(data?.posts || []);
      ...
      <div className="mx-auto w-full max-w-6xl">
        <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between sm:mb-10">
          <div>
            ...
            <p className="mt-2 bb-muted">Your drafts and published posts.</p>
          </div>
        ...
        {status.loading && <p className="bb-muted">Loading posts...</p>}
        {status.error && (
        ...
        {!status.loading && !posts.length && !status.error && (
          <p className="bb-muted">You have not created any posts yet.</p>
        )}
